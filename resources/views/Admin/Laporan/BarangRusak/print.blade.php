...
<body onload="window.print()">
    ...
    <center>
        <h1 class="font-medium">Laporan Barang Rusak</h1>
        @if ($tglawal == '')
        ...
        <tbody>
            ...
            @foreach ($data as $d)
                <tr>
                    ...
                    <td>{{ $d->br_tanggal == '' ? '-' : Carbon::parse($d->br_tanggal)->translatedFormat('d F Y') }}</td>
                    <td>{{ $d->br_kode }}</td>
                    <td>{{ $d->barang_kode }}</td>
                    <td>{{ $d->barang_nama }}</td>
                    <td>{{ $d->jenisbarang_nama }}</td>
                    <td align="center">{{ $d->br_jumlah }}</td>
                    <td>{{ $d->br_keterangan ?? '-' }}</td>
                    <td>{{ $d->br_status }}</td>
                </tr>
            @endforeach
        </tbody>
