    ...
    <script>
        $(document).ready(function() {
            $('#table-1').DataTable({
                ...
                serverSide: true,
                "ajax": {
                    "url": "{{ route('barang-rusak.getbarang-rusak') }}", // Pastikan route ini ada di web.php
                },
                "columns": [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                    ...
                    {
                        data: 'br_jumlah', // Nama kolom asli di Database
                        name: 'br_jumlah',
                    },
                    {
                        data: 'keterangan', // Dari addColumn Controller (alias untuk keterangan)
                        name: 'br_keterangan',
                    },
                    {
                        data: 'action',
        ...
        function generateID() {
            try {
                reset(); // Memanggil fungsi reset dari tambah.blade.php
            } catch (e) {}
            
            // Generate ID Client Side Sederhana
            var id = Math.floor(Math.random() * 1000);
            $('input[name="brkode"]').val("BR-" + id);
